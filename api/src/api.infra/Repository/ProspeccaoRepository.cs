...
namespace api.infra.Repository
{
    public class ProspeccaoRepository : EfRepository<Prospeccao>, IProspeccaoRepository
    {
        ...
        public IEnumerable<Prospeccao> ListarRelatorio(ProspeccaoDTO Prospeccao)
        {
            var ListaCursos = base.Query(x =>
                ...
                .ThenInclude(x => x.Curso).ThenInclude(x=>x.ConteudosProgramaticos)
                .ThenInclude(x => x.ConteudoProgramatico)
                .Include(x => x.Interesses).ThenInclude(x => x.Curso).ThenInclude(x=>x.Docentes)
