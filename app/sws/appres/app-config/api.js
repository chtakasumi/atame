...
api.directive("onEnter",["$injector",function($injector){
	return {
		restrict : 'A',		
		link : function(scope, element, attr) {						
			element.on('keypress',function(events){
				var funcao=angular.element(this).attr('on-enter')
...
/*
api.directive("uiModal",[function() {
	return {
	      template: '<div class="modal fade" visible="false">' + 
	      ...
	      link: function postLink(scope, element, attrs) {
	    	  ...
	    	  element.on("click",function(){
	    		  	...
	    		  	var myElement = $(element).children('div');
		  	      ...
		  	      myElement.on('shown.bs.modal', function(){
		  	          scope.$apply(function(){
		  	            scope.$parent[attrs.visible] = true;
		  	          });
		  	        });
	
		  	    myElement.on('hidden.bs.modal', function(){
		  	          scope.$apply(function(){
...
*/
api.service("upload", [ '$injector', '$timeout','$modal',function($injector,$timeout,$modal) {
	var helper = $injector.get('helper');
	return {		
		open:function(param,callback){						
			...
			$modal.open({
		        ...
		        controller :function($scope, $modalInstance,parametro, FileUploader,helper){
		        	...
		        	$scope.view = { title: "Anexar arquivos", limiteArquivos: parametro.limiteArquivos, dropZone: parametro.dropZone, 
		        			formData: parametro.formData,fileAdd: parametro.fileAdd};
...
api.directive("maskEmail", function ($compile) {
    return {
        restrict: 'A',
        link: function (scope, element, attr) {
          element.inputmask({
...
api.directive('ngAutocomplete', function($injector,$timeout) {
	var helper = $injector.get('helper'); 
    return {
      restrict: 'A',
      scope:{
