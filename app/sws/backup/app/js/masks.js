...
(function() {
	...
	var numericPattern = new StringMask('000000000000,000');
	...
	function uiNumericMask() {
		function applyNumericMask (value) {
			if(!value) {
				return value;
			}
			var formatedValue = numericPattern.apply(value);
			return formatedValue.trim().replace(/[^0-9]$/, '');
		}
		...
		return {
			...
			link: function (scope, element, attrs, ctrl) {
				...
				ctrl.$parsers.push(function(value) {
					...
					var actualNumber = value.replace(/[^\d]/g,'');
					var formatedValue = applyNumericMask(actualNumber);					
	...
	.directive('uiNumericMask', [uiNumericMask])
		...
		return {
			...
			link: function (scope, element, attrs, ctrl) {
				...
				ctrl.$formatters.push(function(value) {
					...
					var resultado =moneyMask.apply(value.replace(/[^\d]+/g,''));;
				...
				ctrl.$parsers.push(function(value) {
					...
					if (value !== formatedValue) {
						formatedValue=(formatedValue===undefined)? 'R$ 0,00':formatedValue;
						ctrl.$setViewValue(formatedValue);
						ctrl.$render();
					}
					
					
					return formatedValue ? formatedValue.replace(/[^\d]+/g,'')/Math.pow(10,decimals) : null; //formatedValue ? parseInt(formatedValue.replace(/[^\d]+/g,''))/Math.pow(10,decimals) : null;
				});
			}
		};
	}])
	.directive('uiBrPhoneMask',function() {
		/**
